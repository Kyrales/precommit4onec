...
Функция ВыполнитьКоманду(Знач ПараметрыКоманды, Знач Приложение) Экспорт
	...
	Лог.Информация("Текущий каталог исходных файлов: %1", ТекущийКаталогИсходныхФайлов);
	...
	Для каждого Изменение Из ЖурналИзменений Цикл
		...
		Если Не ЗначениеЗаполнено(Изменение.ИмяФайла) Тогда
			Продолжить;
		КонецЕсли;
		
		Если Не ФайловыеОперации.ФайлНаходитсяВКаталоге(Изменение.ИмяФайла, ТекущийКаталогИсходныхФайлов) Тогда
			Лог.Информация("Файл для изменения не входит в текущий каталог: %1", Изменение.ИмяФайла);	
			Продолжить;
		КонецЕсли;
...
Процедура ВыполнитьОбработкуФайлов(Файлы, НаборНастроек, ПараметрыОбработки)
	...
	РазрешенныеФайлыОбработкиРегулярныеВыражения = МенеджерНастроек.ЗначениеНастройки("РазрешенныеФайлыОбработки", "", Новый Массив);
	Лог.Отладка("КомандаПрекоммит.ВыполнитьОбработкуФайлов(): РазрешенныеФайлыОбработкиРегулярныеВыражения = " + СтрСоединить(РазрешенныеФайлыОбработкиРегулярныеВыражения,";"));
	ИсключенныеФайлыОбработкиРегулярныеВыражения = МенеджерНастроек.ЗначениеНастройки("ИсключенныеФайлыОбработки", "", Новый Массив);
	Лог.Отладка("КомандаПрекоммит.ВыполнитьОбработкуФайлов(): ИсключенныеФайлыОбработкиРегулярныеВыражения = " + СтрСоединить(ИсключенныеФайлыОбработкиРегулярныеВыражения,";"));
	...
	Пока Ит < Файлы.Количество() Цикл
		...
		Лог.Информация("Анализируется файл <%1>", ИмяФайла);
		...
		Если МенеджерНастроек.ИсключитьФайлИзОбработкиПоНастройкам(АнализируемыйФайл, 
			ИмяФайла, ИсключенныеФайлыОбработкиРегулярныеВыражения, РазрешенныеФайлыОбработкиРегулярныеВыражения) Тогда
			Ит = Ит + 1;
			Продолжить;	
		КонецЕсли;
