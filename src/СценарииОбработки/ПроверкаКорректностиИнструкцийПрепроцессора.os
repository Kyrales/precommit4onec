...
Процедура ПроверитьНаКорректностьИнструкций(ПутьКФайлуМодуля)
	...
	Для Каждого Совпадение Из Совпадения Цикл
		...
		НомерСтроки = 0;
		Если ПустаяСтрока(Инструкция) Тогда
			Ошибки.Добавить(СтрШаблон("Пустая инструкция в строке [%1] в модуле %2", НомерСтрокиСовпадения(НомерСтроки, СодержимоеФайла, Совпадение), ПутьКФайлуМодуля));
		 	Продолжить;
		КонецЕсли;
		...
		Если ЕстьОшибкаПорядкаИнструкции Тогда
			Ошибки.Добавить(СтрШаблон(
					"Нарушен порядок инструкции [%1] в строке [%2] в модуле %3",
					ЧастиИнструкции[0], НомерСтрокиСовпадения(НомерСтроки, СодержимоеФайла, Совпадение), ПутьКФайлуМодуля));
			Прервать;
		КонецЕсли;
		...
		Для Каждого Часть Из ЧастиИнструкции Цикл
			...
			Если Не ПустаяСтрока(Токен) И ВсеТокены.Найти(Токен) = Неопределено Тогда
				Ошибки.Добавить(СтрШаблон(
						"Неизвестная инструкция [%1] в строке [%2] в модуле %3",
						Часть, НомерСтрокиСовпадения(НомерСтроки, СодержимоеФайла, Совпадение), ПутьКФайлуМодуля));
			КонецЕсли;
		КонецЦикла;
	...
	Если Не ЕстьОшибкаПорядкаИнструкции И УровеньЕсли <> 0 Тогда
		ТекстОшибки = СтрШаблон("Нарушена парность инструкций [#Если/#КонецЕсли] в модуле %1", ПутьКФайлуМодуля);
