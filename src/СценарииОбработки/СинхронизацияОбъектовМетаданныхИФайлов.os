...
Функция ОбработатьФайл(АнализируемыйФайл, КаталогИсходныхФайлов, ДополнительныеПараметры) Экспорт
	...
	// Анализ файлов ChartsOfCharacteristicTypes Планов видов характеристик для EDT
	РегулярноеВыраженияПВХ = Новый РегулярноеВыражение("[/\\]ChartsOfCharacteristicTypes[/\\].*\.mdo");
	ЭтоПВХ = РегулярноеВыраженияПВХ.Совпадает(АнализируемыйФайл.ПолноеИмя);
	Если ЭтоПВХ Тогда
		ЭтоEDT = Истина;
		Лог.Информация("Обработка файла '%1' по сценарию '%2 в рамках типов Планов видов характеристик'", АнализируемыйФайл.ПолноеИмя, ИмяСценария());
		Если СинхронизироватьМетаданныеИФайлыПВХ(АнализируемыйФайл.ПолноеИмя, КаталогИсходныхФайлов) Тогда
			ДополнительныеПараметры.ИзмененныеКаталоги.Добавить(АнализируемыйФайл.ПолноеИмя);
		КонецЕсли;
...
//
Функция ПолучитьОбъектыМетаданных(ИмяФайла, ЭтоПВХ = Ложь)
	ОбъектыМетаданных = НоваяТаблицаМетаданных();
	СодержимоеФайла = ФайловыеОперации.ПрочитатьТекстФайла(ИмяФайла);
	Если ЭтоПВХ Тогда
		ДочерниеЭлементы = РегулярныеВыражения.ПолучитьДочерниеЭлементыОписанияПВХ(СодержимоеФайла, ЭтоEDT);	
	Иначе
	...
	Для Каждого ОбъектМетаданных Из ДочерниеЭлементы.Совпадения Цикл
		...
		Если ЭтоПВХ Тогда
			// Преобразование типа данных из ПВХ для возможности корректного поиска в дереве метаданных.
			Если Прав(ТипМетаданных, 3) = "Ref" Тогда
				// Например: Вместо EnumRef будет Enum
				ТипМетаданных = Лев(ТипМетаданных, СтрДлина(ТипМетаданных) - 3);
			Иначе
				// Это примитивный тип (String, Number и т.п.) - физического файла нет.
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		
		НоваяЗапись = ОбъектыМетаданных.Добавить();
		НоваяЗапись.ТипМетаданных = ТипМетаданных;
...
Функция СинхронизироватьМетаданныеИФайлыПВХ(Знач ИмяФайла, Знач КаталогИсходныхФайлов)
	ЭтоПВХ = Истина;
	ОбъектыМетаданных = ПолучитьОбъектыМетаданных(ИмяФайла, ЭтоПВХ);
	Если ОбъектыМетаданных.Количество() = 0 Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Найденныефайлы = НайтиФайлы(КаталогИсходныхФайлов, "Configuration.mdo", Истина);
	Если Найденныефайлы.Количество() = 0 Тогда
        ВызватьИсключение "Configuration.mdo для ПВХ не найден в каталоге исходных файлов.";
    КонецЕсли;
	ИмяФайлаКорня = Найденныефайлы[0].ПолноеИмя;
	КаталогиТиповМетаданных = КаталогиТиповМетаданных(ИмяФайлаКорня, МассивОшибок);
	...
	Для Каждого СтрокаТипа Из ТипыМетаданных Цикл
		...
		МассивОбъектов = МассивОбъектовМетаданных(ОбъектыМетаданных, СтрокаТипа.ТипМетаданных);
		МассивОшибокПриПоискеФайловОбъектов = Новый Массив; // Далее в обработке этот массив ошибок не нужен.
		ФайлыОбъектов = НайтиФайлыОбъектов(КаталогТипа, МассивОбъектов, МассивОшибокПриПоискеФайловОбъектов);
		...
		Для Каждого ОбъектМетаданных Из МассивОбъектов Цикл
			// В массиве остались объекты, по которым не найдены файл описания и каталог объекта
			Ошибка = СтрШаблон("Отсутствуют файлы объекта конфигурации для %1.%2. Ссылаются из плана видов характеристик %3", 
				СтрокаТипа.ТипМетаданных, ОбъектМетаданных, ИмяФайла);
			МассивОшибок.Добавить(Ошибка);
		КонецЦикла;
