...
Функция ПолучитьДочерниеЭлементыОписанияПВХ(Знач СодержимоеФайла, ЭтоEDT) Экспорт
	
	Элементы = Новый Структура("Количество,ДочерниеЭлементыСтрока,Совпадения", 0, "", Неопределено);
	
	Если ЭтоEDT Тогда
		Регексп = Создать("<mdclass:ChartOfCharacteristicTypes[\s\S]*?<type>([\s\S]*?)<\/type>");
	Иначе
		Регексп = Создать("<type>(?:[\s\S]*?)<types>([^<]+)<\/types>");
	КонецЕсли;
	...
	Если НЕ Элементы.Количество = 0 Тогда // Если количество 0 вернется пустая коллекция
		...
		Элементы.ДочерниеЭлементыСтрока = ДочерниеЭлементы[0].Группы[1].Значение;
...
Функция СтрокаСовпадаетСРегулярнымиВыражениями(СтрокаАнализа, МассивРегулярныхВыражений) Экспорт
	
	РезультатСовпадения = Новый Структура;
	РезультатСовпадения.Вставить("Совпадает", Ложь);
	РезультатСовпадения.Вставить("РегулярноеВыражение", "");

	Для Каждого ТекРегВыражениеСтрока Из МассивРегулярныхВыражений Цикл
		
		РегекспМетаданные = Создать(ТекРегВыражениеСтрока);

		РезультатСовпадения.Совпадает = РегекспМетаданные.Совпадает(СтрокаАнализа);

		Если РезультатСовпадения.Совпадает Тогда
			РезультатСовпадения.РегулярноеВыражение = ТекРегВыражениеСтрока;
			Прервать;	
		КонецЕсли;

	КонецЦикла;

	Возврат РезультатСовпадения;
