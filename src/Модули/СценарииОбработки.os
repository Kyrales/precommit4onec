Функция Загрузить(КаталогРепозитория, Проект, Знач ПараметрИменаЗагружаемыхСценариев = Неопределено, Знач ЭтоПринудительноеВыполнениеУказанныхПравил = Ложь) Экспорт
	...
	Если ПараметрИменаЗагружаемыхСценариев <> Неопределено Тогда
		...
		Лог.Отладка("СценарииОбработки.Загрузить(): ИменаЗагружаемыхСценариев взяты из параметра переданного выше: " + СтрСоединить(ИменаЗагружаемыхСценариев,";"));
		
	Иначе
		
		ИменаЗагружаемыхСценариев = МенеджерНастроек.ИменаЗагружаемыхСценариев(Проект);
		Лог.Отладка("СценарииОбработки.Загрузить(): ИменаЗагружаемыхСценариев инициалазированы с нуля, т.к. выше НЕ было определенных сценариев: " + СтрСоединить(ИменаЗагружаемыхСценариев,";"));
	...
	Если НЕ МенеджерНастроек.ЭтоНовый() Тогда
			...
			Иначе
				...
				Если Не ЭтоПринудительноеВыполнениеУказанныхПравил Тогда
					...
					Для каждого ТекФайлЛокальногоСценария Из ФайлыЛокальныхСценариевДляОбработки Цикл
						...
						Если ИменаИсключаемыхСценариев.Найти(ТекФайлЛокальногоСценария.ИмяБезРасширения) = Неопределено
							И ИменаИсключаемыхСценариев.Найти(ТекФайлЛокальногоСценария.Имя) = Неопределено  Тогда
		...
		КонецЕсли; // ИспользоватьСценарииРепозитория

	КонецЕсли; // НЕ МенеджерНастроек.ЭтоНовый()
	...
	Лог.Отладка("СценарииОбработки.Загрузить(): Для ФайлыГлобальныхСценариев ИменаЗагружаемыхСценариев: " + СтрСоединить(ИменаЗагружаемыхСценариев,";"));
	ЗагрузитьИзКаталога(ВсеЗагруженные, ФайлыГлобальныхСценариев, ИменаЗагружаемыхСценариев);
	Лог.Отладка("СценарииОбработки.Загрузить(): Для ФайлыЛокальныхСценариев ИменаЗагружаемыхСценариев: " + СтрСоединитьИменаФайлов(ФайлыЛокальныхСценариев,";"));
...
Функция НастройкаОбработкиПроекта(ИмяПроекта, КаталогРепозитория, Знач ИменаЗагружаемыхСценариев = Неопределено, Знач ЭтоПринудительноеВыполнениеУказанныхПравил = Ложь)
	...
	Если ИменаЗагружаемыхСценариев = Неопределено Тогда
		...
		Лог.Отладка("СценарииОбработки.НастройкаОбработкиПроекта(): ИменаЗагружаемыхСценариев = Неопределено, выполнена инициализция сценариев из глобальных настроек. Итог: ИменаЗагружаемыхСценариев = " + СтрСоединить(ИменаЗагружаемыхСценариев,";"));
	КонецЕсли;
	
	Настройка.СценарииОбработки = Загрузить(КаталогРепозитория,
			ИмяПроекта,
			ИменаЗагружаемыхСценариев, 
			ЭтоПринудительноеВыполнениеУказанныхПравил);
...
Функция СтрСоединитьИменаФайлов(МассивФайлов, Разделитель) Экспорт
	
	ТекущаяСтрока = "";
	
	Для каждого ТекЗначение Из МассивФайлов Цикл
		ТекущаяСтрока = ТекущаяСтрока + ТекЗначение.ИмяБезРасширения + Разделитель;	
	КонецЦикла;
