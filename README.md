...
```JSON
{
    ...
    "Precommt4onecСценарии": {
        ...
        "РазрешенныеФайлыОбработки": [...],         // Фильтр разрешенных к обработке полных имен файлов (поддерживаются регулярные выражения). Пример: \\\\Префикс_ - фильтр по значению "\Префикс_" . Имеет больший приоритет, чем ИсключенныеФайлыОбработки
        "ИсключенныеФайлыОбработки": [...],         // Фильтр исключенных к обработке полных имен файлов (поддерживаются регулярные выражения). РазрешенныеФайлыОбработки приоритетней, чем ИсключенныеФайлыОбработки. 
...
### Отладка сценариев обработок и тестов

[См. подробнее](/docs/ОтладкаСценариев.md)
