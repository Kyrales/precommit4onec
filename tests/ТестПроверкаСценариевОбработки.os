...
Функция ПолучитьСписокТестов(ЮнитТестирование) Экспорт
	...
	ВсеТесты.Добавить("ТестДолжен_ПроверитьЧтоСинхронизацияОбъектовМетаданныхДляПВХВызываетИсключение");	
	...
	ВсеТесты.Добавить("ТестДолжен_ПроверитьЧтоСценарийИсправлениеНеКаноническогоНаписанияИгнорируетМодулиРасширенияСКонтролемИзменений");
	...
	ВсеТесты.Добавить("ТестДолжен_ПроверитьЧтоСообщениеКоммитаСоответствуетМаске");
...
Процедура ТипыФайлов_ЗащищенныеМодулиНеОпределяютсяКакФайлИсходников() Экспорт
	ПутьКФайлу = ОбъединитьПути(КаталогИсходников(), ФайловыеОперации.НормализоватьРазделители("tests\fixtures\ЗащищенныеФайлы"), "Module.bsl");
...
Процедура ТестДолжен_ПроверитьЧтоСценарийИсправлениеНеКаноническогоНаписанияИгнорируетМодулиРасширенияСКонтролемИзменений() Экспорт
	ОбъектСценария = ПолучитьСценарий("ИсправлениеНеКаноническогоНаписания.os");
	ИмяФайла = ФайловыеОперации.НормализоватьРазделители("МодулиРасширения/МодульСКонтролемИзменений.bsl");
	
	Файл = ПодготовитьИзменяемыйТестовыйФайл(ИмяФайла, "МодульСКонтролемИзменений.bsl");
	ТекстФайлаДо = ФайловыеОперации.ПрочитатьТекстФайла(Файл.ПолноеИмя);
	Результат = ОбъектСценария.ОбработатьФайл(Файл, Файл.Путь, ПолучитьДополнительныеНастройки());
	ТекстФайлаПосле = ФайловыеОперации.ПрочитатьТекстФайла(Файл.ПолноеИмя);
	
	Ожидаем.Что(ТекстФайлаДо, "Файл не был изменен").Равно(ТекстФайлаПосле);
КонецПроцедуры
...
Процедура ТестДолжен_ПроверитьЧтоСинхронизацияОбъектовМетаданныхВызываетИсключение() Экспорт
	...
	ВременныйКаталог = ПолучитьИмяВременногоФайла();
	Фикстура = Фикстура("СинхронизацияОбъектовМетаданныхИФайлов");
	СоздатьКаталог(ВременныйКаталог);
	СкопироватьКаталог(Фикстура, ВременныйКаталог);

	ПутьККаталогу = ФайловыеОперации.НормализоватьРазделители(ОбъединитьПути(ВременныйКаталог, "Conf/ПустойКаталог"));
	...
	Попытка
		// Формат EDT
		ПутьКФайлу = ОбъединитьПути(ВременныйКаталог, "EDT/src/Configuration/Configuration.mdo");
		Файл = Новый Файл(ФайловыеОперации.НормализоватьРазделители(ПутьКФайлу));
		
		МетаданныеКонфигурации.СброситьКеш();
		ОбъектСценария.ОбработатьФайл(Файл, Файл.Путь, ДополнительныеНастройки);
	Исключение
	...
	Попытка
		...
		ПутьКФайлу = ОбъединитьПути(ВременныйКаталог, "Conf/Configuration.xml");
		Файл = Новый Файл(ФайловыеОперации.НормализоватьРазделители(ПутьКФайлу));
...
Процедура ТестДолжен_ПроверитьЧтоСинхронизацияОбъектовМетаданныхДляПВХВызываетИсключение() Экспорт
	ОбъектСценария = ПолучитьСценарий("СинхронизацияОбъектовМетаданныхИФайлов.os");
	ДополнительныеНастройки = ПолучитьДополнительныеНастройки();
	...
	Попытка
		// Формат EDT
		ПутьКИсходнымФайлам = ФайловыеОперации.НормализоватьРазделители(ОбъединитьПути(ВременныйКаталог, "EDT/src"));
		ПутьКФайлу = ОбъединитьПути(ВременныйКаталог, "EDT/src/ChartsOfCharacteristicTypes/ХарактеристикиПользователей/ХарактеристикиПользователей.mdo");
		Файл = Новый Файл(ФайловыеОперации.НормализоватьРазделители(ПутьКФайлу));
		
		МетаданныеКонфигурации.СброситьКеш();
		ОбъектСценария.ОбработатьФайл(Файл, ПутьКИсходнымФайлам, ДополнительныеНастройки);
	Исключение
		ТекстОшибки = ОписаниеОшибки();
	КонецПопытки;

	Ожидаем.Что(ТекстОшибки, "Отсутствует физический файл метаданных указанный в типе ПВХ").Содержит("Отсутствуют файлы объекта конфигурации для Catalog.НесуществующийСправочник");
...
#Область ПроверкаСообщенияКоммита

Процедура ТестДолжен_ПроверитьЧтоСообщениеКоммитаСоответствуетМаске() Экспорт
	Фикстура = Фикстура("ПроверкаСообщенияКоммита");
	ВременныйКаталог = ПолучитьИмяВременногоФайла();
	СоздатьКаталог(ВременныйКаталог);
	СкопироватьКаталог(Фикстура, ВременныйКаталог);

	РепозиторийGit = Новый ГитРепозиторий();
	РепозиторийGit.УстановитьРабочийКаталог(ВременныйКаталог);
	РепозиторийGit.Инициализировать();

	Приложение = МенеджерПриложения.Инициализировать(ПараметрыПриложения);
	Параметры = Новый Соответствие;
	Параметры.Вставить("КаталогРепозитория", ВременныйКаталог);
	Параметры.Вставить("-source-dir", "");
	
	ТекстСообщения = "ORAIS-1234: Проверка содержимого сообщения коммита";
	ФайловыеОперации.ЗаписатьТекстФайла(ОбъединитьПути(ВременныйКаталог, ".git", "COMMIT_EDITMSG"), ТекстСообщения);

	ПоложительныйРезультат = Приложение.ВыполнитьКоманду(ПараметрыПриложения.ИмяКомандыПоУмолчанию(), Параметры);

	ТекстСообщения = "Сообщение коммита без номера задачи";
	ФайловыеОперации.ЗаписатьТекстФайла(ОбъединитьПути(ВременныйКаталог, ".git", "COMMIT_EDITMSG"), ТекстСообщения);

	ОтрицательныйРезультат = Приложение.ВыполнитьКоманду(ПараметрыПриложения.ИмяКомандыПоУмолчанию(), Параметры);

	МенеджерВременныхФайлов.УдалитьФайл(ВременныйКаталог);

	Ожидаем.Что(ПоложительныйРезультат).Равно(Приложение.РезультатыКоманд().Успех);
	Ожидаем.Что(ОтрицательныйРезультат).Равно(Приложение.РезультатыКоманд().ОшибкаВремениВыполнения);
КонецПроцедуры


Процедура ТестДолжен_ПроверитьЧтоСообщениеКоммитаОбрабатываютФайлыТолькоВУказанномКаталоге() Экспорт

	Фикстура = Фикстура("ПроверкаПрекоммитаНаОбработкуФайлов");
	ВременныйКаталог = ПолучитьИмяВременногоФайла();
	СоздатьКаталог(ВременныйКаталог);
	...
	РепозиторийGit.УстановитьРабочийКаталог(ВременныйКаталог);
	РепозиторийGit.Инициализировать();
	РепозиторийGit.ДобавитьФайлВИндекс(".");
	РепозиторийGit.Закоммитить("Первый коммит");
	...
	Параметры = Новый Соответствие;
	Параметры.Вставить("КаталогРепозитория", ВременныйКаталог);
	Параметры.Вставить("-source-dir", "src"); // Только из этого каталога могут браться на проверку изменяемые данные
	
	ТекстСообщения = "Проверка содержимого сообщения коммита в конкретном каталоге src";
	ФайловыеОперации.ЗаписатьТекстФайла(ОбъединитьПути(ВременныйКаталог, ".git", "COMMIT_EDITMSG"), ТекстСообщения);

	// выполняем команду проверки пре-коммита
	ПоложительныйРезультат = Приложение.ВыполнитьКоманду(ПараметрыПриложения.ИмяКомандыПоУмолчанию(), Параметры);

	Ожидаем.Что(ПоложительныйРезультат).Равно(Приложение.РезультатыКоманд().Успех);

	ОтносительныйПутьКФайлу = "src\1\ИсправлениеНеКаноническогоНаписания1.bsl";
	ФайлДляОбработки = Новый Файл(ОбъединитьПути(ВременныйКаталог, ОтносительныйПутьКФайлу));
	СодержимоеФайла1 = СокрЛП(ФайловыеОперации.ПрочитатьТекстФайла(ФайлДляОбработки.ПолноеИмя));

	ОтносительныйПутьКФайлу = "ИсправлениеНеКаноническогоНаписания3.bsl";
	ФайлДляОбработки = Новый Файл(ОбъединитьПути(ВременныйКаталог, ОтносительныйПутьКФайлу));
	СодержимоеФайла2 = СокрЛП(ФайловыеОперации.ПрочитатьТекстФайла(ФайлДляОбработки.ПолноеИмя));
	
	МенеджерВременныхФайлов.УдалитьФайл(ВременныйКаталог);
	
	// Проверка факта изменения файла в каталоге src
	Ожидаем.Что(СодержимоеФайла1, "Файл не должен быть равен файлу из корня, т.к. должен был быть изменен")
		.Не_().Равно(СодержимоеФайла2);
...
#Область ПроверкаЛокальныхСценариев

// Проверяется выполнение командой exec-rules строго указанного сценария в случае включенных локальных сценариев (без глобальных)
// Например: Если указано "precommit4onec exec-rules -source-dir src/cf -rules "КорректировкаXMLФорм" .",
// то должно выполниться только КорректировкаXMLФорм, даже если в локальных сценариях еще есть сценарии
Процедура ТестДолжен_ПроверитьЧтоВыполняютсяПринудительноУказанныеЛокальныеСценарии() Экспорт
	
	// 1. Подготовка данных
	ПараметрыПодготовки = ПодготовкаБазовыхПараметровДляПринудительногоВыполнения("КорректировкаXMLФорм");

	ВременныйКаталогПосле = ПараметрыПодготовки.ВременныйКаталогПосле; 
	ВременныйКаталогДо    = ПараметрыПодготовки.ВременныйКаталогДо; 
	
	// 2. Выполнение функции	
	Приложение = МенеджерПриложения.Инициализировать(ПараметрыПриложения);
	ПоложительныйРезультат = Приложение.ВыполнитьКоманду("exec-rules", ПараметрыПодготовки.ПараметрыКоманды);
	
	// 3. Проверка результата
	Ожидаем.Что(ПоложительныйРезультат).Равно(Приложение.РезультатыКоманд().Успех);

	СодержимоеФайловДляПроверки = СодержимоеФайловПослеПринудительногоВыполненияСценариев(ВременныйКаталогДо, ВременныйКаталогПосле);

	// Проверка измененного файла по сценарию КорректировкаXMLФорм.
	Ожидаем.Что(СодержимоеФайловДляПроверки.СодержимоеФайлаПервогоПосле, "Файл не должен быть равен эталону, т.к. должен был быть изменен")
		.Не_().Равно(СодержимоеФайловДляПроверки.СодержимоеПервогоЭталон);

	// Проверка, что файл с некорректными пробелами не был изменен, т.к. УдалениеЛишнихКонцевыхПробелов не было вызвано.
	Ожидаем.Что(СодержимоеФайловДляПроверки.СодержимоеФайлаВторогоПосле, "Файл должен быть равен эталону, т.к. в рамках локальных сценариев не перечислен для выполнения")
		.Равно(СодержимоеФайловДляПроверки.СодержимоеВторогоЭталон);

	МенеджерВременныхФайлов.УдалитьФайл(ВременныйКаталогДо);
	МенеджерВременныхФайлов.УдалитьФайл(ВременныйКаталогПосле);

КонецПроцедуры

// Проверяется выполнение командой exec-rules без указания сценариев, в случае включенных локальных сценариев (без глобальных)
// Будуть браться все локальные сценарии
Процедура ТестДолжен_ПроверитьЧтоВыполняютсяВсеЛокальныеСценарии() Экспорт
		
		// 1. Подготовка данных
		БезСценариев = ""; // в этом случае должны браться, все локальные сценарии
		ПараметрыПодготовки = ПодготовкаБазовыхПараметровДляПринудительногоВыполнения(БезСценариев);
		...
		// Проверка, что файл с некорректными пробелами не был изменен, т.к. УдалениеЛишнихКонцевыхПробелов не было вызвано.
		Ожидаем.Что(СодержимоеФайловДляПроверки.СодержимоеФайлаВторогоПосле, "Файл не должен быть равен эталону, т.к. должен был быть изменен")
			.Не_().Равно(СодержимоеФайловДляПроверки.СодержимоеВторогоЭталон);
...
Функция ПодготовкаБазовыхПараметровДляПринудительногоВыполнения(ИменаСценариев = "")
	
	РезультатПодготовки = Новый Структура;
	РезультатПодготовки.Вставить("ВременныйКаталогПосле", ПолучитьИмяВременногоФайла()); // BSLLS:MissingTemporaryFileDeletion-off
	РезультатПодготовки.Вставить("ВременныйКаталогДо", ПолучитьИмяВременногоФайла()); // BSLLS:MissingTemporaryFileDeletion-off

	ВременныйКаталогПосле = РезультатПодготовки.ВременныйКаталогПосле; 
	ВременныйКаталогДо    = РезультатПодготовки.ВременныйКаталогДо; 

	Фикстура = Фикстура("ПринудительноеВыполнениеСценариев");
	СоздатьКаталог(ВременныйКаталогДо);
	СкопироватьКаталог(Фикстура, ВременныйКаталогДо);	
	СоздатьКаталог(ВременныйКаталогПосле);
	СкопироватьКаталог(Фикстура, ВременныйКаталогПосле);

	ПараметрыКоманды = Новый Соответствие;
	ПараметрыКоманды.Вставить("КаталогРепозитория", ВременныйКаталогПосле);
	ПараметрыКоманды.Вставить("-source-dir", "");
	ПараметрыКоманды.Вставить("-rules", ИменаСценариев);		

	РезультатПодготовки.Вставить("ПараметрыКоманды", ПараметрыКоманды);

	Возврат РезультатПодготовки;

КонецФункции

Функция СодержимоеФайловПослеПринудительногоВыполненияСценариев(ВременныйКаталогДо, ВременныйКаталогПосле)
	
	ОтносительныйПутьКИзменяемомуФайлу = "src\BadForm\Form.form";
	ФайлПервыйДо 	  = Новый Файл(ОбъединитьПути(ВременныйКаталогДо, ОтносительныйПутьКИзменяемомуФайлу));
	ФайлПервыйПосле = Новый Файл(ОбъединитьПути(ВременныйКаталогПосле, ОтносительныйПутьКИзменяемомуФайлу));
	
	ОтносительныйПутьКНеизменяемомуФайлу = "src\DelProbels\Module.bsl";
	ФайлВторойДо 	  = Новый Файл(ОбъединитьПути(ВременныйКаталогДо, ОтносительныйПутьКНеизменяемомуФайлу));
	ФайлВторойПосле = Новый Файл(ОбъединитьПути(ВременныйКаталогПосле, ОтносительныйПутьКНеизменяемомуФайлу));

	// Файлы для сценария КорректировкаXMLФорм.
	СодержимоеФайлаПервогоПосле = СокрЛП(ФайловыеОперации.ПрочитатьТекстФайла(ФайлПервыйПосле.ПолноеИмя));
	СодержимоеПервогоЭталон = СокрЛП(ФайловыеОперации.ПрочитатьТекстФайла(ФайлПервыйДо.ПолноеИмя));
	
	// Файлы для сценария УдалениеЛишнихКонцевыхПробелов.
	СодержимоеФайлаВторогоПосле = СокрЛП(ФайловыеОперации.ПрочитатьТекстФайла(ФайлВторойПосле.ПолноеИмя));
	СодержимоеВторогоЭталон = СокрЛП(ФайловыеОперации.ПрочитатьТекстФайла(ФайлВторойДо.ПолноеИмя));

	ПараметрыФайлов = Новый Структура;
	ПараметрыФайлов.Вставить("СодержимоеФайлаПервогоПосле", СодержимоеФайлаПервогоПосле);
	ПараметрыФайлов.Вставить("СодержимоеПервогоЭталон", СодержимоеПервогоЭталон);
	ПараметрыФайлов.Вставить("СодержимоеФайлаВторогоПосле", СодержимоеФайлаВторогоПосле);
	ПараметрыФайлов.Вставить("СодержимоеВторогоЭталон", СодержимоеВторогоЭталон);
